...
## Verified locally on 2026-08-05 — Sector Sprint one-hit jetpack gauntlet

- Sector Sprint keeps five ordered 32-second Acts and the foreground-only 240-second House boundary, but its Action route is now a deterministic jetpack gauntlet. A 900 ms ignition hover leads into constant gravity, bounded upward thrust, preserved glide inertia, and capped rise/fall speeds. One swept contact with the road, ceiling, or authored corridor face creates a neutral, non-persistent wipeout.
- The five Acts contain 8, 9, 10, 11, and 12 paired passages. Clearances tighten from 132 to 116 CSS pixels and cadence tightens from 3.0 to 2.2 seconds. Sandstone, market timber, hammered brass, wet terrazzo, and phulkari inlay have distinct seams, hardware, lattice, chips, edge light, and depth treatment; the quiet render tier removes expensive texture and shadow without changing collision geometry.
- Gurpreet, Harjit, and their compact duo formation share velocity pitch, bounded pose blending, rise/fall squash, scarf and bag lag, a visible flight rig, and a failure sputter. Reduced motion bypasses Action for the complete narrated route, so precision movement, Canvas animation, vision, and sound remain unnecessary for completion.
- Recovery removes the underlying Action controls and moves keyboard focus to Retry when a provably complete five-Act attempt still fits, including five full two-second catch-up allowances, otherwise to Continue narrated. Retry reuses the same run and foreground boundary. The boundary remains active during failure and Act transitions, outranks final-Act completion, and records no Gallery completion when it closes. Failure, collision, input, timing, retry, and performance history are never persisted; optional audio is suspended before idle recovery.
- The refreshed landing copy, README, public contract, accessibility page, testing page, roadmap, and 1280×619 built-source capture describe the one-hit corridor rather than the superseded non-failing leap/dash route. The earlier dated sections below remain historical delivery records, not current behavior.
- `npm run check` passes with zero Astro diagnostics, 80 unit tests, and the composed production build. The complete `npm test` matrix passes across standalone House, root/base-path public pages, one-hit recovery, all five rendered materials, motion-state evidence, final-transition closure, narration, keyboard/focus, offline PWA, same-origin privacy, Night isolation, and the existing Night/Dawn gates.
- The latest complete Chromium game-feel run at 375×812 under 4× CPU slowdown measured pulse-to-render p95 21.4 ms and dense-corridor frame p95 34.9 ms; the 1440×900 desktop frame p95 was 16.8 ms. Enforced ceilings remain 150 ms input response, 50 ms throttled-phone frame interval, and 25 ms desktop frame interval.
- Automated geometry and controller evidence establishes a substantially tighter deterministic challenge, not Flappy Bird-equivalent difficulty. Native Mobile Safari pacing, physical-device thermals/battery, real assistive-technology use, human cultural-authenticity review, biomechanical realism, comparative human difficulty, and representative adult judgments of fun remain unverified.
