...
## Verified locally on 2026-08-04 — Grand Salon motion and recovery polish

- All five Grand Salon tables now have a deterministic presentation signature: inlaid glyph choreography for Pattern Court, compass-ring turns for Mirror Forge, lift-and-settle discs for Stack Architect, a named light procession with a velvet cover for Lantern Ledger, and an explicit Action/Narrated route prelude for Sector Sprint. The refreshed House capture is rendered from the built source.
- The new recovery flows are bounded and local. An unfinished first-four table reload offers Continue, Start over, or Exit; meaningful progress asks before a table is abandoned; Stack reset affects only the current tower; Lantern replay reveals the same fixed procession and disables answers until it is covered again. Chapter and Act transitions pause behind the exit confirmation, Keep playing resumes the held transition, Leave table records no completion, and cancel restores focus.
- Sector Sprint creates no run, session marker, Canvas loop, or foreground boundary before a route is chosen. Action and Narrated retain the same five Acts, 32-second Act boundary, 240-second foreground-active table boundary, completion facts, compact storage whitelist, and reload-fail-closed rule. Reduced-motion visitors still enter the complete narrated route directly.
- Motion is decorative and deterministic. Wrong-answer copy, live status, visible selection, semantic controls, and fixed game state provide the same information without animation or sound; reduced motion removes the four new game signatures and keeps Sector Sprint on narration. No new runtime asset, request, identifier, telemetry, score, streak, random reward, or Night Room state was added.
- `npm run check` passes with zero Astro diagnostics, 78 unit tests, and the composed build. `npm run test:house`, `npm run test:runner-feel`, `npm run test:public`, `npm run test:pwa`, `npm run test:portrait`, and `npm run test:layered` pass across five-game completion, recovery, keyboard/focus, 320px-through-desktop rendering, root/base-path public surfaces, House cache `nindova-house-v6`, cold offline operation, Night standalone/PWA isolation, Rasoi layouts, and reduced motion.
- The current Chromium game-feel gate at 375×812 under 4× CPU slowdown measured p95 Leap 3.5 ms, Dash 3.3 ms, Tool 4.2 ms, and frame interval 33.8 ms; desktop frame interval was 18.1 ms. Native Mobile Safari pacing, real-device assistive technology, physical-device battery/thermals, human cultural-authenticity review, and representative adult judgments of enjoyment or “coolness” remain unverified.
