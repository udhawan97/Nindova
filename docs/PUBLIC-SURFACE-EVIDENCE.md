...
- `npm run check` passes with zero Astro diagnostics and 101 unit tests. The complete rendered correctness matrix passes root/base-path public surfaces, two repeated full House journeys, all eight completions, independent standalone/PWA behavior, Dawn, Night state, layered play, deterministic closure, portrait/zoom/reduced motion, tile latency, House/Night cache migration, and HTTP-cache-cleared cold-offline entry.
