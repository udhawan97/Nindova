...
- Representative Chicago, Kolkata, and daylight-saving Night IDs each produce 382 Gentle and 517 Deeper reachable states, one terminal state per profile, and zero dead states.
...
- The completed Session makes Rest primary and offers one optional, response-free Image Drift with no route back into play; browser evidence closes the page, opens a new page in the same profile, and completes the second touch Session without multiplying Dawn.
...
On 2026-08-03, `npm run test:wall-clock` exercised the production application without reviewer acceleration. The Session reached `end` at 722.388 seconds through the `production-cap` path, reached `rest` by 900.001 observed seconds, and reported no console or page errors.
