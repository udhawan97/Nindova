...
- **Sector Sprint — vertical-slice implementation complete; target-browser acceptance pending.** Render-only interpolation, richer authored city layers, stronger rider presence, and deterministic local feedback are implemented, and the automated latency/frame ceilings pass. Fresh Safari acceptance is still required by the Phase A exit gate; physical iPhone and human feel review remain Phase E evidence gates.
- **Stack Architect — bespoke action pass complete in the current candidate.** The selected top disc lifts from its real plinth, a brass datum arc traces the legal source-to-destination traverse, and the destination disc compresses before settling. The board now reads as a cabinetmaker's rosewood instrument at phone and desktop widths. Saved state, legality, chapter order, completion, keyboard operation, and reduced-motion behavior are unchanged.
- **Lantern Ledger — bespoke action pass complete in the current candidate.** Individually shaped glass lamps now ignite in authored order along a numbered brass ledger rail, resolve into one even-glow reading, and disappear behind a two-panel velvet screen before the answer becomes available. Visible sequence truth, answer gating, reveal, reload/restore, chapter order, keyboard operation, and reduced-motion completion are unchanged. Automated phone, tablet, desktop, full-House, and PWA gates pass; fresh Safari capture remains pending because local browser control was unavailable.
