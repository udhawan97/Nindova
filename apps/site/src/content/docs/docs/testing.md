...
- `test:house` completes every chapter or Act of all five Salon games through production controls; checks Sector Sprint action auto-advance and full closure, exact whole-table boundary without false completion, 239-second reload fail-closed behavior, blur/hidden/pause suspension during Acts and transitions, paused-narration completion without chime, action controls, global pointer cancellation, narrated and reduced-motion routes, compact active storage, 320px/desktop runner rendering, keyboard/focus and nonvisual Stack state, adult-boundary behavior, result replacement, corrupt recovery, Gallery clearing, responsive layouts, same-origin requests, direct Night isolation, and a cold offline navigation after clearing the HTTP cache.
