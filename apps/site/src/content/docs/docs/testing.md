...
- `test:house` completes every chapter or Act of all five Salon games through production controls; checks Sector Sprint pulse/glide input, deterministic one-contact recovery, conservative same-boundary retry, exact whole-table closure during failure and final-Act transition, reload fail-closed behavior, blur/hidden/pause suspension, optional-audio release, all five rendered corridor materials, rise/fall/impact evidence, the narrated and reduced-motion routes, compact active storage, 320px/desktop rendering, keyboard/focus and nonvisual Stack state, adult-boundary behavior, result replacement, corrupt recovery, Gallery clearing, responsive layouts, same-origin requests, direct Night isolation, and cold offline navigation.
- `test:runner-feel` drives the live sandstone corridor with a deterministic pulse controller, measures six pulse-to-first-render samples at 375×812 under 4× CPU throttling, and samples 120 sustained frame intervals under that phone profile and at 1440×900.
...
- `test:layered` proves real visual overlap and z-order, the structural Gentle/Deeper distinction, Deeper's one opening pair, covered/side-blocked semantics, synchronous pair removal, the paired bloom, qualitative path reflection, optional Image Drift, reduced-motion opacity, Rest, and deliberate same-night return.
