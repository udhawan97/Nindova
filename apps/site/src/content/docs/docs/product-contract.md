...
- Sector Sprint is a fixed-route jetpack table, not an endless runner. Its Action route uses automatic forward motion, bounded pulse-and-glide physics, five deterministic architectural corridors, five Act-local harmless tools, and one neutral wipeout when the shared collision hull touches the road, ceiling, or corridor face. A wipeout stores no failure history and creates no score, high score, visible timer, health, lives, checkpoint, progress reward, randomized reward, persistent inventory, or claimed sleep effect. People, animals, vehicles, and buildings can never be targets. Retry uses the same foreground-only table boundary; the complete narrated route has the same entertainment provenance and curtain call. If the boundary arrives before every Act closes, it outranks transition or recovery input and exits without a Gallery completion.
...
- Adult acknowledgement is local audience framing, not identity or date-of-birth verification.

## Night Room contract

- One Rasoi Pairs rule and two equal tonight-only board profiles inside the Night Room; no rounds, progression levels, unlocks, or escalating difficulty.
...
The intake promise is: “Nothing to win. Nothing tracked. Nothing you can do wrong.”

The end card begins: “The session is over. That's the point.”

Nindova House is intended for adults 18 and over. The Night Room is a behavioral design study, not a sleep tracker, sleep-performance tool, memory intervention, or treatment. Masala Mound has not been clinically shown to make people sleepy, improve sleep, improve general memory, or create a useful dopamine response at any age. Persistent sleep difficulty deserves evidence-based care such as CBT-I with a qualified clinician.
