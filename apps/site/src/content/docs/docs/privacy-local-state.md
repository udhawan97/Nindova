...
The `/house/` worker precaches the core versioned static House graph in `nindova-house-v12` so a fresh controlled navigation works with the HTTP cache cleared and the network offline. The optional Sector Sprint illustration is cached only after Action requests it; if that first request is offline, the existing vector lead keeps the route complete. The `/play/` worker owns the separately scoped Night shell in `nindova-session-v5`. Neither worker can read local storage or cache state keys, Dawn blobs, or share payloads. The standalone `nindova.html` registers no worker.
