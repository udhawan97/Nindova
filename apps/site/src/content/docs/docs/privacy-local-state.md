...
`nindova:house:active:v1` in session storage contains the current game, chapter, local run identifier, Lantern cover state, Stack plinth state, Sector Sprint narrated-story beat, or one session-only `touched` boolean used solely to decide whether leaving needs confirmation. Pattern Court also stores its current nine-cell board, remaining tray, selected piece, and at most 18 board-and-tray snapshots for Undo. This move-level Pattern state supports same-tab recovery; it is cleared with the active record when the game ends and disappears when the tab session ends. None of it enters the completion record. Lantern and Stack also use the active snapshot for same-tab reload recovery. Sector Sprint position, elapsed time, inputs, power-ups, ribbons, bag contents, choices, and interaction history remain in memory and are never persisted; because persisting a clock would create performance history, any Sector Sprint document reload fails closed to the Salon, clears the active record, records no completion, and explains the boundary exit. Corrupt or illegal Pattern Court or Stack state resets to its authored opening.
...
The `/house/` worker precaches the core versioned static House graph in `nindova-house-v15` so a fresh controlled navigation works with the HTTP cache cleared and the network offline. The full original Chandigarh world and character atlas are included, so the first offline walk uses the complete art. The `/play/` worker owns the separately scoped Night shell in `nindova-session-v5`. Neither worker can read local storage or cache state keys, Dawn blobs, or share payloads. The standalone `nindova.html` registers no worker.
