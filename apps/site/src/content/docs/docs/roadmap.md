...
The pinned GitHub Pages workflow publishes the static build from `main`. The deployed `main` revision exposes the live House, Night, docs, and standalone routes. v0.4.0 packages that complete static surface. The rendered QR destination, service-worker control, and offline reload are browser-verified; a physical-device QR scan remains pending.
