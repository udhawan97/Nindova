...
- Sector Sprint, an original code-drawn Chandigarh lane route led by Gurpreet and Harjit, with five fixed Acts, expressive illustrated riders, layered city depth, automatic progressively faster motion, three discrete lanes, five increasingly dense material gate sequences, one neutral non-persistent Action pause, five Act-local harmless tools, a complete narrated route, no score/lives/checkpoints, an honest no-completion boundary exit, and a deterministic curtain call.
...
- Deterministic brass-and-peacock match feedback, an optional three-note chime, and a reduced-motion opacity alternative.
...
The pinned GitHub Pages workflow publishes the static build from `main`. The deployed `main` revision exposes the live House, Night, docs, and standalone routes. This public-surface refresh remains local until authorized delivery. The latest tagged download is still v0.3.0 and packages the earlier Night Room release rather than the later House. The rendered QR destination, service-worker control, and offline reload are browser-verified; a physical-device QR scan remains pending.
