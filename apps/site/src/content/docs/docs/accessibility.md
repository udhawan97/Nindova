...
Sector Sprint's Canvas route advances automatically and exposes native Leap/Air step, Dash/Stomp, Act tool, Pause, and Narrated route controls. The one-button movement grammar includes input buffering, coyote time, a generous held-height window, one air step, contextual low vault, and aerial stomp; none is required to complete the route. Collisions create one presentation-only response and never change logical position, velocity, input access, route timing, completion, or progress. A large external next-interference cue, nonnumeric action-status labels, and the live region report Canvas information in text without announcing every frame. Reduced motion starts directly in the rewritten complete three-beat-per-Act text route, which reaches the same deterministic curtain call and requires no timed response, precision, sound, animation, or visual interpretation; speech and sound never autoplay. Pause holds active time for either route while narrated beat controls remain available and Pause persists between narrated Acts; paused narration does not play the optional chime. An unfinished route that reaches the absolute boundary or reloads without its in-memory clock closes without claiming completion.
